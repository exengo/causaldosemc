...
cdmc_refit_baseline <- function(object, drop_mask, rerun_tuning = FALSE, verbose = FALSE) {
  ...
  lambda_selection_result <- select_fit_lambda(
    ...
    cv_workers = fit_control$workers %||% fit_control$cv_workers %||% 1L,
    cv_top_k = fit_control$cv_top_k %||% NULL,
    cv_coarse_to_fine = fit_control$cv_coarse_to_fine %||% FALSE,
    cv_coarse_nlambda = fit_control$cv_coarse_nlambda %||% NULL,
    cv_warm_starts = fit_control$cv_warm_starts %||% FALSE,
    rank_max = rank_max,
